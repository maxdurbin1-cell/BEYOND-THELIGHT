...
(function () {
  ...
  function stampCrucibleExpeditionProvinceFeatures(map, day) {
    if (!map || !map.hexes) return { ruins: [], portals: [], gates: [] };
  ...
  function ensureProvinceEndgameExpeditionState() {
    ensureNewFeatureState();
    if (!S.endgameProvinceExpedition || typeof S.endgameProvinceExpedition !== 'object') {
      S.endgameProvinceExpedition = {
        active: false,
        completed: false,
        day: 1,
        travelClicks: 0,
    ...
    var st = S.endgameProvinceExpedition;
    if (!st.collapsed || typeof st.collapsed !== 'object') st.collapsed = {};
    st.day = Math.max(1, Number(st.day || 1));
    st.travelClicks = Math.max(0, Number(st.travelClicks || 0));
    st.collapseDepth = Math.max(0, Number(st.collapseDepth || 0));
    st.maxRing = Math.max(0, Number(st.maxRing || 0));
    st.clicksPerCollapse = Math.max(2, Number(st.clicksPerCollapse || 3));
    st.bossReady = !!st.bossReady;
    st.active = !!st.active;
    st.completed = !!st.completed;
    st.startedAt = Number(st.startedAt || 0);
    st.lastCollapsedAt = String(st.lastCollapsedAt || '');
    return st;
  }

  function getProvinceMapSnapshotSafe() {
    if (typeof window.getProvinceMapState !== 'function') return null;
    try {
      return window.getProvinceMapState() || null;
    } catch (_err) {
      return null;
    }
  }

  function getProvinceMapDataSafe() {
    var snap = getProvinceMapSnapshotSafe();
    return (snap && Array.isArray(snap.mapData)) ? snap.mapData : [];
  }

  function getProvinceBounds(mapData) {
    ...
    mapData.forEach(function (hex) {
      if (!hex) return;
      var c = Number(hex.col || 0);
      var r = Number(hex.row || 0);
  ...
  function provinceHexKey(col, row) {
    return String(Number(col || 0)) + ',' + String(Number(row || 0));
  }

  function provinceRingIndex(hex, bounds) {
    if (!hex || !bounds) return 0;
    var col = Number(hex.col || 0);
    var row = Number(hex.row || 0);
    return Math.min(
      Math.max(0, col - bounds.minCol),
      Math.max(0, bounds.maxCol - col),
      Math.max(0, row - bounds.minRow),
      Math.max(0, bounds.maxRow - row)
    );
  }

  function isProvinceEndgameExpeditionCollapsedHex(hex) {
    var st = ensureProvinceEndgameExpeditionState();
    if (!st.active || !hex) return false;
    var key = provinceHexKey(hex.col, hex.row);
    return !!st.collapsed[key];
  }

  function maybeRelocateProvinceExpeditionSelection(st, mapData) {
    var snap = getProvinceMapSnapshotSafe();
    var currentKey = snap ? String(snap.selectedKey || '') : '';
    if (!currentKey || !st.collapsed[currentKey]) return;
    var parts = currentKey.split(',');
    var originCol = Number(parts[0]);
    var originRow = Number(parts[1]);
    var safe = null;
    var best = Infinity;
    mapData.forEach(function (hex) {
      if (!hex) return;
      var key = provinceHexKey(hex.col, hex.row);
      if (st.collapsed[key]) return;
      var dist = Math.abs(Number(hex.col || 0) - originCol) + Math.abs(Number(hex.row || 0) - originRow);
      if (dist < best) {
        best = dist;
        safe = hex;
      }
    });
    if (safe && typeof window.setProvinceSelectedKey === 'function') {
      window.setProvinceSelectedKey(provinceHexKey(safe.col, safe.row));
      if (typeof showNotif === 'function') showNotif('The collapsing frontier forced a retreat to a safer hex.', 'warn');
    }
  }

  function collapseProvinceEndgameExpeditionRing(st) {
    var mapData = getProvinceMapDataSafe();
    if (!mapData.length) return 0;
    var bounds = getProvinceBounds(mapData);
    if (!bounds) return 0;
    if (Number(st.maxRing || 0) <= 0) {
      st.maxRing = Math.max(1, Math.floor(Math.min(bounds.width, bounds.height) / 2));
    }
    var snap = getProvinceMapSnapshotSafe();
    var selectedKey = snap ? String(snap.selectedKey || '') : '';
    var targetRing = Math.max(0, Number(st.collapseDepth || 0));
    ...
    mapData.forEach(function (hex) {
      ...
      var key = provinceHexKey(hex.col, hex.row);
      if (st.collapsed[key]) return;
      if (provinceRingIndex(hex, bounds) !== targetRing) return;
      if (key === selectedKey) return;
      st.collapsed[key] = true;
      collapsedCount += 1;
    });
    ...
    st.collapseDepth = targetRing + 1;
    st.lastCollapsedAt = new Date().toISOString();
    if (st.day < 2 && st.collapseDepth >= 1) {
      st.day = 2;
      st.clicksPerCollapse = 2;
    }

    maybeRelocateProvinceExpeditionSelection(st, mapData);

    var remaining = mapData.filter(function (hex) {
      if (!hex) return false;
      return !st.collapsed[provinceHexKey(hex.col, hex.row)];
    }).length;
    if (remaining <= Math.max(3, Math.ceil(mapData.length * 0.08))) st.bossReady = true;
    if (Number(st.collapseDepth || 0) >= Number(st.maxRing || 1)) st.bossReady = true;
    return collapsedCount;
  }

  function onProvinceEndgameExpeditionTravel(hexClicks) {
    var st = ensureProvinceEndgameExpeditionState();
    if (!st.active) return;
    var steps = Math.max(1, Number(hexClicks || 1));
    st.travelClicks = Math.max(0, Number(st.travelClicks || 0) + steps);
    var threshold = Math.max(2, Number(st.clicksPerCollapse || 3));
    var collapsedTotal = 0;
    while (st.travelClicks >= threshold) {
      st.travelClicks -= threshold;
      collapsedTotal += collapseProvinceEndgameExpeditionRing(st);
      threshold = Math.max(2, Number(st.clicksPerCollapse || 3));
    }
    if (collapsedTotal > 0 && typeof showNotif === 'function') {
      showNotif('The frontier collapsed by ' + collapsedTotal + ' hexes.', 'warn');
    }
  }

  function renderProvinceEndgameExpeditionHud() {
    var panelHost = document.getElementById('tab-map');
    if (!panelHost) return;
    var controls = panelHost.querySelector('.map-controls');
    var panel = document.getElementById('provinceExpeditionModePanel');
    if (!panel) {
      panel = document.createElement('div');
      panel.id = 'provinceExpeditionModePanel';
      panel.style.margin = '.25rem 0 .1rem';
      panel.style.padding = '.35rem .45rem';
      panel.style.border = '1px solid var(--border2)';
      panel.style.background = 'rgba(255,132,80,.08)';
    ...
    var collapsedCount = Object.keys(st.collapsed || {}).length;
    var threshold = Math.max(2, Number(st.clicksPerCollapse || 3));
    var remainingToCollapse = Math.max(0, threshold - Number(st.travelClicks || 0));
    panel.innerHTML = '<div style="display:flex;gap:.3rem;align-items:center;justify-content:space-between;flex-wrap:wrap;">'
      + '<div style="font-size:.74rem;line-height:1.45;color:var(--text2);">'
      + '<strong style="color:var(--gold2);">Province Expedition Mode</strong>'
      + ' · Day ' + Number(st.day || 1)
      + ' · Collapsed: ' + collapsedCount
      + ' · Next collapse in ' + remainingToCollapse + ' travel click' + (remainingToCollapse === 1 ? '' : 's')
      + (st.bossReady ? ' · <span style="color:var(--red2);">Raid boss is ready.</span>' : '')
      + '</div>'
      + '<div style="display:flex;gap:.22rem;flex-wrap:wrap;">'
      + (st.bossReady ? '<button class="btn btn-xs btn-warn" onclick="launchProvinceEndgameRaidBoss()">Launch Final Raid Boss</button>' : '')
      + '<button class="btn btn-xs btn-red" onclick="stopProvinceEndgameExpedition()">Stop Mode</button>'
      + '</div>'
      + '</div>';
  }

  function renderProvinceEndgameExpeditionMapOverlay() {
    renderProvinceEndgameExpeditionHud();
    var st = ensureProvinceEndgameExpeditionState();
    if (!st.active) return;
    ...
    if (typeof window.hexToPixel !== 'function' || typeof window.hexPoints !== 'function') return;
    var size = Number(window.HEX_SIZE || 30);

    mapData.forEach(function (hex) {
      if (!hex) return;
      var key = provinceHexKey(hex.col, hex.row);
      if (!st.collapsed[key]) return;
      var pix = window.hexToPixel(Number(hex.col || 0), Number(hex.row || 0));
      var g = document.createElementNS('http://www.w3.org/2000/svg', 'g');
      g.setAttribute('pointer-events', 'none');

      var poly = document.createElementNS('http://www.w3.org/2000/svg', 'polygon');
      poly.setAttribute('points', window.hexPoints(pix.x, pix.y, size - 1));
      poly.setAttribute('fill', 'rgba(14,14,20,.86)');
      poly.setAttribute('stroke', 'rgba(220,90,90,.82)');
      poly.setAttribute('stroke-width', '1.5');
      g.appendChild(poly);

      var l1 = document.createElementNS('http://www.w3.org/2000/svg', 'line');
      l1.setAttribute('x1', String(pix.x - size * 0.42));
      l1.setAttribute('y1', String(pix.y - size * 0.42));
      l1.setAttribute('x2', String(pix.x + size * 0.42));
      l1.setAttribute('y2', String(pix.y + size * 0.42));
      l1.setAttribute('stroke', 'rgba(245,120,120,.92)');
      l1.setAttribute('stroke-width', '1.6');
      g.appendChild(l1);

      var l2 = document.createElementNS('http://www.w3.org/2000/svg', 'line');
      l2.setAttribute('x1', String(pix.x + size * 0.42));
      l2.setAttribute('y1', String(pix.y - size * 0.42));
      l2.setAttribute('x2', String(pix.x - size * 0.42));
      l2.setAttribute('y2', String(pix.y + size * 0.42));
      l2.setAttribute('stroke', 'rgba(245,120,120,.92)');
  ...
  function installProvinceEndgameExpeditionHooks() {
    if (window._provinceEndgameExpeditionHooksInstalled) return;
    window._provinceEndgameExpeditionHooksInstalled = true;

    if (typeof window.registerProvinceHexTravel === 'function') {
      var baseRegisterProvinceHexTravel = window.registerProvinceHexTravel;
      window.registerProvinceHexTravel = function (hexClicks) {
        var out = baseRegisterProvinceHexTravel.apply(this, arguments);
        try { onProvinceEndgameExpeditionTravel(hexClicks); } catch (_err) {}
        return out;
      };
    }

    if (typeof window.isTraversalPhaseLocked === 'function') {
      var baseTraversalLock = window.isTraversalPhaseLocked;
      window.isTraversalPhaseLocked = function (targetHex) {
        if (isProvinceEndgameExpeditionCollapsedHex(targetHex)) {
          if (typeof showNotif === 'function') showNotif('That frontier hex has collapsed and cannot be entered.', 'warn');
          return true;
        }
        return baseTraversalLock.apply(this, arguments);
      };
    }

    if (typeof window.renderHexMap === 'function') {
      var baseRenderHexMap = window.renderHexMap;
      window.renderHexMap = function () {
        var out = baseRenderHexMap.apply(this, arguments);
        try { renderProvinceEndgameExpeditionMapOverlay(); } catch (_err) {}
        return out;
      };
    }
  }

  function startProvinceEndgameExpedition() {
    ensureNewFeatureState();
    installProvinceEndgameExpeditionHooks();
    if (!Array.isArray(getProvinceMapDataSafe()) || !getProvinceMapDataSafe().length) {
    ...
    var mapData = getProvinceMapDataSafe();
    if (!mapData.length) {
      if (typeof showNotif === 'function') showNotif('Generate a Province first, then start Expedition Mode.', 'warn');
      return false;
    }
    ...
    st.collapseDepth = 0;
    st.collapsed = {};
    st.clicksPerCollapse = 3;
    st.bossReady = false;
    st.startedAt = Date.now();
    st.lastCollapsedAt = '';

    var bounds = getProvinceBounds(mapData);
    st.maxRing = bounds ? Math.max(1, Math.floor(Math.min(bounds.width, bounds.height) / 2)) : 3;

    var snap = getProvinceMapSnapshotSafe();
    var selectedKey = snap ? String(snap.selectedKey || '') : '';
    if (!selectedKey && mapData[0] && typeof window.setProvinceSelectedKey === 'function') {
      window.setProvinceSelectedKey(provinceHexKey(mapData[0].col, mapData[0].row));
    }

    if (typeof window.switchTab === 'function') {
      var btn = document.getElementById('tabnav-map');
      window.switchTab('map', btn || null);
    }
    if (typeof window.renderHexMap === 'function') window.renderHexMap();
    renderProvinceEndgameExpeditionHud();
    if (typeof showNotif === 'function') showNotif('Province Expedition Mode active. Travel as normal; frontier edges will collapse over time.', 'good');
    return true;
  }

  function stopProvinceEndgameExpedition() {
    var st = ensureProvinceEndgameExpeditionState();
    st.active = false;
    st.travelClicks = 0;
    st.collapseDepth = 0;
    ...
    renderProvinceEndgameExpeditionHud();
    if (typeof window.renderHexMap === 'function') window.renderHexMap();
    if (typeof showNotif === 'function') showNotif('Province Expedition Mode stopped.', 'info');
    return true;
  }

  function launchProvinceEndgameRaidBoss() {
    var st = ensureProvinceEndgameExpeditionState();
    if (!st.active || !st.bossReady) {
      if (typeof showNotif === 'function') showNotif('Raid boss is not ready yet. Keep advancing through collapsing hexes.', 'warn');
      return false;
    }
    if (typeof window.holdingCrucibleSetMode === 'function') {
      window.holdingCrucibleSetMode('expedition');
    }
    if (typeof window.openHoldingCrucibleMatch === 'function') {
      window.openHoldingCrucibleMatch('expedition');
      return true;
    }
    if (typeof showNotif === 'function') showNotif('Raid boss launcher is unavailable right now.', 'warn');
    return false;
  }
  ...
  window.startProvinceEndgameExpedition = startProvinceEndgameExpedition;
  window.stopProvinceEndgameExpedition = stopProvinceEndgameExpedition;
  window.launchProvinceEndgameRaidBoss = launchProvinceEndgameRaidBoss;
